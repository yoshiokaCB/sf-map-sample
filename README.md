...
```bash
response-check$ sam logs -n StockCheckerFunction --stack-name response-check --tail
```
...
```bash
response-check$ ruby tests/unit/test_stock_checker.rb
```
...
```bash
aws cloudformation delete-stack --stack-name response-check
```
